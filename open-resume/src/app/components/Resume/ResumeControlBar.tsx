...
const ResumeControlBar = ({
  ...
  const handleExport = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (window !== window.parent) {
      // In an iframe -> send to parent
      const resumeData = loadStateFromLocalStorage()?.resume || {};
      ...
      // Not in iframe -> standard download
      if (instance.url) {
  ...
  return (
    <div className="sticky bottom-0 left-0 right-0 flex h-[var(--resume-control-bar-height)] items-center justify-center px-[var(--resume-padding)] text-gray-600 lg:justify-between">
      ...
      >
        ...
        <span className="whitespace-nowrap">Export to Workify</span>
      </a>
    </div>
  );
};
