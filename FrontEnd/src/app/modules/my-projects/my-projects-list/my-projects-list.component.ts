...
import {
  ...
  ProjectStatus,
  ProjectCategory,
} from '../../../core/models/my-project.model';
import { MyProjectsService } from '../../../core/services/my-projects.service';
import { AuthService } from '../../../core/services/auth.service';
import { MlService, PredictResponse } from '../../../core/services/ml.service';
...
export class MyProjectsListComponent implements OnInit {
  ...
  loadProjects(): void {
    ...
    const load$ = (this.canManageProjects && userEmail)
      ? this.myProjectsService.getMyProjects(userEmail)
      : this.myProjectsService.getAllProjects();
  ...
  openCreateForm(): void {
    ...
    this.formModel.clientName = this.authService.getUserName() ?? '';
    this.formModel.clientEmail = this.authService.getUserEmail() ?? '';
    this.formError = '';
  }
  ...
  predictBudget(): void {
    ...
    this.budgetPredicting = true;
    this.budgetPrediction = null;

    const tags = [category, ...title.toLowerCase().split(' ')].filter(t => t.length > 2);

    this.mlService.predict({ job_title: title, tags, rate_type: 'fixed' }).subscribe({
      next: (res) => { this.budgetPrediction = res; this.budgetPredicting = false; },
      error: () => { this.budgetPredicting = false; }
    });
  }

  applyPredictedBudget(): void {
    if (this.budgetPrediction) {
      this.formModel.minBudget = Math.round(this.budgetPrediction.predicted_avg);
      this.formModel.maxBudget = Math.round(this.budgetPrediction.predicted_avg);
    }
  }
...
submitForm(): void {
  ...
  const apiProjectData: ApiProject = {
    ...
    clientEmail: this.formModel.clientEmail || null,
    clientPhone: this.formModel.clientPhone || null,
    clientId: this.authService.getNumericUserId() ?? null,
    createdAt: null,
    status: this.mapDisplayStatusToApi(this.formModel.status),
  };
  ...
  confirmDelete(): void {
    if (!this.canManageProjects || !this.pendingDelete?.id) {
      return;
    }
    this.myProjectsService.deleteProject(this.pendingDelete.id).subscribe({
      next: () => {
  ...
  private mapDisplayStatusToApi(displayStatus: string | null): string | null {
